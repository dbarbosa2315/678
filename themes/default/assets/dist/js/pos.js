function add_invoice_item(item) {
    ...
    var item_id = item.item_id + '_' + item.variant_id;
...
function loadItems() {
    ...
    if (get('spositems')) {
        ...
        var total_tems = 0;
        $.each(spositems, function(i, item) {
            total_tems += parseFloat(item.row.qty);
        });
                
        $.each(spositems, function (item_id, item) {
            ...
            if (total_tems < 6 && item.row.real_unit_price > 0) {
                unit_price += parseFloat(item.row.retail_tax);
            }
            ...
            var row_no = String(Math.random()).replace('0.', '');
            
            var newTr = $('<tr id="' + row_no + '" class="' + item_id + '" data-item-id="' + item_id + '"></tr>');
            var tr_html = '<td>\n\
            <a href="#" class="edit" id="' + row_no + '" data-item="' + item_id + '">\n\
                <span class="sname" id="name_' + row_no + '">' + item_code + ' - ' + item_name.substring(0, 25) + '</span>\n\
            </a>\n\
            <input name="product_id[]" type="hidden" class="rid" value="' + product_id + '">\n\
            <input name="variant_id[]" type="hidden" value="' + item.variant_id + '">\n\
            <input name="exchange_val[]" type="hidden" value="'+ item.row.exchange_val +'">\n\
            </td>';
            // <input class="rprice" name="net_price[]" type="hidden" id="price_' + row_no + '" value="' + formatDecimal(item_price) + '">
            tr_html += '<td class="text-right"><input class="realuprice" name="real_unit_price[]" type="hidden" value="' + unit_price + '"><input class="rdiscount" name="product_discount[]" type="hidden" id="discount_' + row_no + '" value="' + ds + '"><span class="text-right sprice" id="sprice_' + row_no + '">' + formatMoney(parseFloat(net_price) + parseFloat(pr_tax_val)) + '</span></td>';
            tr_html += '<td class="text-center"><input class="form-control input-qty kb-pad text-center rquantity" name="quantity[]" type="text" value="' + formatDecimal(item_qty) + '" data-id="' + row_no + '" data-item="' + item_id + '" id="quantity_' + row_no + '" onClick="this.select();" onkeypress="update_quantity(event)" step="1"></td>';
...
$(document).ready(function(){
    ...
    $("#posTable").on("click", '.edit', function() {
        ...
        $('#nDiscount').val(0);
        $('#split-item').iCheck('uncheck');
        $('#item_sale_paid').val(0);
    ...
    $("#add_item").autocomplete({
        ...
        response: function (event, ui) {
            ...
            else if (ui.content.length == 1 && ui.content[0].id != 0 && $("#add_item").val() === ui.content[0].row.ean) {
    ...
    $('#payment').click(function () {
        if (count <= 1) {
            ...
            if (!(seller > 0)) {
                bootbox.alert('Selecione o vendedor');
                return false;
            }
            ...
            var total = $('#total-payable').text().trim().replace(',', '');
                
            $('#amount').val(total);
            $('#balance_ori').val(total);
    ...
    $('#submit-sale').click(function(ev) {
        $('#total_items').val(an - 1);
        $('#total_quantity').val(count - 1);
        
        if (balance_val < 0) {
            bootbox.alert('Pagamento incompleto');
            return false;
        }
        ...
        $('#abort-sale').attr('disabled', true);
                
        $('#amount_val').val(total_paying - balance_val);
