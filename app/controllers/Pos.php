...
class Pos extends MY_Controller {
    ...
    function __construct() {
        ...
        if (!$this->loggedIn && strpos($this->router->uri->uri_string, 'pos/view/') !== 0) {
            redirect('login');
        }
    ...
    function index($sid = NULL, $eid = NULL) {
        ...
        foreach ($this->cards_model->getAllTax()as $tax) {
            $tax_cards[$tax->type] = $tax->tax_client / 100;
            $this->data["tax_$tax->type"] = $tax->tax_client;
        }

        $this->data['tax_cards'] = json_encode($tax_cards);
        ...
        if ($this->form_validation->run() == true) {
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                if (isset($item_id) && isset($real_unit_price) && isset($item_quantity)) {
                    ...
                    $products[] = array(
                        ...
                        'exchange_val' => floatval($_POST['exchange_val'][$r]),
                        ...
                        'subtotal' => $subtotal,
                        'real_unit_price' => ($_POST['exchange_val'][$r] > 0) ? $_POST['exchange_val'][$r] : $real_unit_price,
            ...
            foreach ($post['method'] as $i => $method) {
                $data["total_$method"] += $post['amount_row'][$i];

                if ($method === 'credit') {
                    $data['instalments_credit'] = $post['instalments'][$i];
                }

                $data['grand_total'] += floatval($post['amount_row'][$i]);
            }
        ...
        if ($this->form_validation->run() == true && !empty($products)) {
            if ($suspend) {
                ...
                if (get_cookie("sale_$key") === "1") {
                    $this->session->set_flashdata('error', 'Venda duplicada');
                    redirect("pos");
                }

                set_cookie("sale_$key", "1", 600);
            ...
            $this->data['sellers'] = $this->sellers_model->getActive();
    ...
    function suggestions() {
        ...
        if ($rows) {
            ...
            foreach ($rows as $row) {
                ...
                $pr[$row->id] = array(
                    'id' => str_replace(".", "", microtime(true) + mt_rand(1, 100)),
                    'item_id' => $row->id,
                    'variant_id' => 0,
            ...
            foreach ($pr as $id => $row) {
                ...
                foreach ($variants as $variant) {
                    ...
                    $data[] = array(
                        ...
                        'label' => "{$row['row']->code} - $variant->size - $variant->color",
                        'row' => $row['row'],
    ...
    function view($sale_id = NULL, $noprint = NULL) {
        ...
        $this->data['seller'] = $this->sellers_model->get($inv->seller_id);
    ...
    public function printCupom($id) {

        $script = FCPATH . "infra/phantomjs/screenshot.js";

        $url = base64_encode(site_url("pos/view/$id/1"));

        $file = base64_encode(FCPATH . "infra/tmp/cupom_$id.pdf");
        ...
        $file = 'infra\tmp\cupom_' . $id;

        exec('"C:\Program Files\PowerShell\7\pwsh.exe" -ExecutionPolicy Bypass -InputFormat none -File infra\print.ps1 ' . $file . '> print_ok.txt 2> print_erro.txt', $out, $code);

        if (file_exists("infra/tmp/cupom_$id.pdf") && $code === 0) {
            exit("0");
        } else {
            var_dump($code);
            var_dump($out);
