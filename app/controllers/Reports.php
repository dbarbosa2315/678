...
//COALESCE(sum(".$this->db->dbprefix('sale_items').".quantity)*".$this->db->dbprefix('products').".cost, 0) as cost,
    ...
    function estoque() {
        ...
        $bc = array(array('link' => '#', 'page' => lang('reports')), array('link' => '#', 'page' => 'Relatório de Estoque'));
        ...
        $json = consumirApi('relatorioEstoque', $post);

        $resp = json_decode($json);

        if ($resp->sucesso) {
            $this->data['thead'] = $resp->dados;
        }
        ...
        $this->page_construct('reports/estoque', $this->data, $meta);
    }
