...
class Pos_model extends CI_Model
{
    ...
    {
        $this->db->where("code LIKE '" . $term . "%' OR ean LIKE '" . $term . "%'");
        $this->db->limit($limit);
    ...
    {
        $data['seq_id'] = $this->getSequenceId();
        ...
        if ($this->db->insert('sales', $data)) {
            ...
            foreach ($items as $item) {
                ...
                $save = $this->db->insert('sale_items', $item);
                
                //baixa estoque via integracao bling
                if ($save && CODIGO_LOJA != 'ONLINE') {
                    
                    $product = $this->site->getProductByID($item['product_id']);
                    
                    if ($product->type == 'standard') {
                        
                        $set = array('quantity' => $product->quantity - $item['quantity']);
                        
                        if ($item['exchange_val'] > 0) {
                            $set = array('quantity' => $product->quantity + $item['quantity']);
                        }
                        
                        $this->db->update('products', $set, array('id' => $product->id));
                    ...
                    /*
                    $this->db->where(array('id' => $item['variant_id']));
    ...
    public function getAllSaleItems($sale_id)
    {
        $this->db->select('sale_items.*, products.name as product_name, products.code as product_code, products.tax_method as tax_method, '
                . 'products_variants.color as vcolor, products_variants.size as vsize')
            ->join('products', 'products.id=sale_items.product_id')
            ->join('products_variants', 'products_variants.id=sale_items.variant_id', 'LEFT')
    ...
    public function getSequenceId() {
        ...
        $table = 'sales_sequence';

        $day = date('Y-m-d');

        $query = $this->db->get_where($table, ['day' => $day], 1);

        if ($query->num_rows() > 0) {

            $id = $query->row()->id + 1;

            $this->db->update($table, ['id' => $id], ['day' => $day]);
        ...
        $this->db->insert($table, ['id' => 1, 'day' => $day]);
